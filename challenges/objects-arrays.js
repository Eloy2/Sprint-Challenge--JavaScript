...
tyra.roar = "RAWERSRARARWERSARARARRRR!";
...
graduates.map((i) => {
  return universities.push(i.university);
})
...
graduates.map((i) => {
  return contactInfo.push(`${i.first_name} ${i.email}`);
})
...
zooAnimals.map((i) => {
  return lowCaseAnimalNames.push(`${i.animal_name.toLowerCase()}`);
})
...
zooAnimals.filter((i) => {
  if (i.population < 5) {
    return lowPopulationAnimals.push(i);
  }
})
...
zooAnimals.reduce((accu, curr) => {
  return populationTotal = accu + curr.population;
},0);
